...
function solveOne(data: string): any {
  return parseBlueprints(data).reduce((sum, blueprint, i) => {
    const score = bfs(
      blueprint.ore,
      blueprint.clay,
      blueprint.obsidian,
    ...
    return sum + score * (i + 1);
  }, 0);
}

(() => {
  const data = fs.readFileSync(__dirname + "/input/test1.txt").toString();
...
// (() => {
//   const data = fs.readFileSync(__dirname + "/input/input.txt").toString();
